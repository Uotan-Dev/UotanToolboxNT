...
namespace UotanToolbox.Common.PatchHelper
{
    internal class MagiskPatch
    {
        ...
        public static async Task<string> Magisk_Patch(ZipInfo zipInfo, BootInfo bootInfo)
        {
            ...
            var config_path = Path.Combine(bootInfo.TempPath, "config");
            ...
            var randomStr = new string(Enumerable.Repeat(allowedChars, 16)
                .Select(s => s[random.Next(s.Length)]).ToArray());

            var configContent = $"RANDOMSEED=0x{randomStr}";
            ...
            var newboot = Path.Combine(Path.GetDirectoryName(bootInfo.Path), "magisk_patched_" + randomStr + ".img");
            File.Copy(Path.Combine(bootInfo.TempPath, "new-boot.img"), newboot, true);
            return newboot;
        }

        static void boot_img_pre(BootInfo bootinfo)
        {
        ...
        static async Task patched_img_pre(BootInfo bootinfo)
        {
        ...
        static async Task comp_copy(ZipInfo zipInfo, BootInfo bootinfo)
        {
            try
            {
                var archSubfolder = bootinfo.Arch switch
                {
                    "aarch64" => "arm64-v8a",
                    "armeabi" => "armeabi-v7a",
                    "X86" => "x86",
                ...
                var compPath = Path.Join(zipInfo.TempPath, "lib", archSubfolder);
        ...
        static async Task ramdisk_patch(BootInfo bootInfo)
        {
            string mb_output;
            int exitcode;
        ...
        static async Task kernel_patch(BootInfo bootInfo, bool LEGACYSAR)
        {
            if (bootInfo.HaveKernel)
            {
                var kernel_patched = false;
        ...
        static async Task dtb_patch(BootInfo bootInfo)
        {
            if (bootInfo.HaveDTB)
            {
        ...
        static void CleanBoot(string path)
        {
