...
namespace UotanToolbox.Common
{
    internal class FeaturesHelper
    {
        ...
        {
            await CallExternalProgram.ADB($"-s {device} push Push/mkfs.f2fs /tmp/");
            await CallExternalProgram.ADB($"-s {device} shell chmod +x /tmp/mkfs.f2fs");
            await CallExternalProgram.ADB($"-s {device} push Push/mkntfs /tmp/");
            await CallExternalProgram.ADB($"-s {device} shell chmod +x /tmp/mkntfs");
        }

        public static async Task GetPartTable(string device)
        {
            await CallExternalProgram.ADB($"-s {device} push Push/parted /tmp/");
        ...
        {
            await CallExternalProgram.ADB($"-s {device} push Push/parted /data/local/tmp/");
