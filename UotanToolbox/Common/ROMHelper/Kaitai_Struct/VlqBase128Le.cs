    ...
    public partial class VlqBase128Le : KaitaiStruct
    {
        ...
        public int Value
        {
            get
            {
                if (f_value)
                    return _value;
                _value = Groups[0].Value + (Len >= 2 ? Groups[1].Value << 7 : 0) + (Len >= 3 ? Groups[2].Value << 14 : 0) + (Len >= 4 ? Groups[3].Value << 21 : 0) + (Len >= 5 ? Groups[4].Value << 28 : 0) + (Len >= 6 ? Groups[5].Value << 35 : 0) + (Len >= 7 ? Groups[6].Value << 42 : 0) + (Len >= 8 ? Groups[7].Value << 49 : 0);
                f_value = true;
                return _value;
            }
        }
